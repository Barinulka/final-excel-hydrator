...
* FinancialModel — aggregate root для расчетного сценария
    * принадлежит Project
    * управляет названием, versionNumber, shortId, sourceModelId и статусом модели
...
* User — aggregate root для identity/access
    ...
    * пользовательская настройка AmountDisplayFormat определяет формат отображения сумм
